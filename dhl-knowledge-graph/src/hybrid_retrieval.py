...
def build_hybrid_evidence(graph_context: dict, vector_documents: list[dict]) -> list[dict]:
    ...
    for rule in graph_context["applicable_customs_rules"]:
        evidence.append({
            "source": "Graph", "evidence_type": "Applicable rule", "id": rule.get("rule_id", "unknown-rule"),
            "title": rule.get("name", "Unnamed rule"), "detail": rule.get("description", "No description recorded"),
    ...
    for case in graph_context["historical_similar_cases"]:
        evidence.append({
            "source": "Graph", "evidence_type": "Similar case", "id": case.get("case_id", "unknown-case"),
            "title": case.get("issue", "Unspecified issue"), "detail": case.get("resolution", "No resolution recorded"),
    ...
    for document in vector_documents:
        ...
        evidence.append({
            ...
            "title": metadata.get("title", "Untitled document"), "detail": document.get("text", ""),
